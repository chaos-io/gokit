package metrics

import (
	"context"
	"strings"
	"time"

	"github.com/prometheus/client_golang/prometheus"
	"google.golang.org/grpc"
	"google.golang.org/grpc/status"
)

type GRPCServerMetrics struct {
	enabled  bool
	requests *prometheus.CounterVec
	duration *prometheus.HistogramVec
	inflight *prometheus.GaugeVec
}

func NewGRPCServer(provider *Provider) *GRPCServerMetrics {
	m := &GRPCServerMetrics{enabled: provider.Enabled()}
	if !m.enabled {
		return m
	}
	m.requests = prometheus.NewCounterVec(prometheus.CounterOpts{
		Namespace: provider.Namespace(), Name: "grpc_server_requests_total", Help: "gRPC requests completed.",
	}, []string{"service", "method", "code"})
	m.duration = prometheus.NewHistogramVec(prometheus.HistogramOpts{
		Namespace: provider.Namespace(), Name: "grpc_server_request_duration_seconds",
		Help: "gRPC request duration.", Buckets: prometheus.DefBuckets,
	}, []string{"service", "method"})
	m.inflight = prometheus.NewGaugeVec(prometheus.GaugeOpts{
		Namespace: provider.Namespace(), Name: "grpc_server_requests_in_flight", Help: "gRPC requests in flight.",
	}, []string{"service", "method"})
	provider.MustRegister(m.requests, m.duration, m.inflight)
	return m
}

func (m *GRPCServerMetrics) UnaryInterceptor() grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (any, error) {
		if !m.enabled {
			return handler(ctx, req)
		}
		service, method := splitGRPCMethod(info.FullMethod)
		started := time.Now()
		m.inflight.WithLabelValues(service, method).Inc()
		defer m.inflight.WithLabelValues(service, method).Dec()
		resp, err := handler(ctx, req)
		m.requests.WithLabelValues(service, method, status.Code(err).String()).Inc()
		m.duration.WithLabelValues(service, method).Observe(time.Since(started).Seconds())
		return resp, err
	}
}

func splitGRPCMethod(full string) (string, string) {
	parts := strings.Split(strings.TrimPrefix(full, "/"), "/")
	if len(parts) != 2 {
		return "unknown", "unknown"
	}
	return parts[0], parts[1]
}
